...
/**
 * {@link EventExecutorGroup} which wraps another {@link EventExecutorGroup} which contains <strong>NO</strong>
 * {@link OrderedEventExecutor}s and provide ordering while not guarantee that the execution {@link Thread} not changes
 * between excecution.
 */
public final class NonStickyEventExecutorGroup implements EventExecutorGroup {
    ...
    public EventExecutor next() {
        return new NonStickyOrderedEventExecutor(group.next());
    }

    @Override
    public Iterator<EventExecutor> iterator() {
        final Iterator<EventExecutor> executors = group.iterator();
        return new Iterator<EventExecutor>() {
            @Override
            public boolean hasNext() {
                return executors.hasNext();
            }

            @Override
            public EventExecutor next() {
                return new NonStickyOrderedEventExecutor(executors.next());
            }
